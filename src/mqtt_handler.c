...
#include "logging_levels.h"
    ...
    #define LIBRARY_LOG_NAME     "MQTT_HANDLER"
#endif
...
char* getTopic(int topicIndex){
    switch (topicIndex)
    {
    case 0:
        return MQTT_ACTIVATE_LICENSE_REQUEST_TOPIC;    
    case 1:
...
int getTopicLen(int topicIndex){
    switch (topicIndex)
    {
    case 0:
        return MQTT_ACTIVATE_LICENSE_REQUEST_TOPIC_LENGTH;    
    case 1:
                                   ...
                                   uint16_t packetIdentifier )
{
    ...
    if( ( pPublishInfo->topicNameLength == getTopicLen(topicIndex) ) &&
        ( 0 == strncmp( getTopic(topicIndex),
...
static int subscribeToTopic( MQTTContext_t * pMqttContext )
{
    ...
    pGlobalSubscriptionList[ 0 ].pTopicFilter = getTopic(topicIndex);
    pGlobalSubscriptionList[ 0 ].topicFilterLength = getTopicLen(topicIndex);
...
static int publishToTopic( MQTTContext_t * pMqttContext )
{
    ...
    else
    {
        ...
        outgoingPublishPackets[ publishIndex ].pubInfo.pTopicName = getTopic(topicIndex);
        outgoingPublishPackets[ publishIndex ].pubInfo.topicNameLength = getTopicLen(topicIndex);
...
static int subscribeRecive( MQTTContext_t* pMqttContext ){
    int returnStatus = EXIT_SUCCESS;
    MQTTStatus_t mqttStatus = MQTTSuccess;
...
static void prepareActivationResponse(){
